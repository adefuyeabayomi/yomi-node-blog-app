        ...
        <div class="buffer-container">
            <div class="sidebar-container">
                <div class="tools-container">
                    ...
                    <div class="tool-icon-container mb-4">
                        <vs-tooltip danger>
                            <vs-button danger flat @click="deleteDraft()">
            ...
            <div class="edit-contents-container p-4">
                <div class="center">
                    <h2 class="text-h2">Edit Draft</h2>
                ...
                <div class="render-contents-container">
                        ...
                        <div class="edit-title-container fine-box">
                            <div v-if="!editMode || editMode && editingCurrent !== 'title'" class="render pointercursor" title="Post Title, Click To Edit This Section" @click="goToEditMode('title')">
                                <div class="post-title-container">
                                    <div>
                                        <p class="text-h6">Title : "{{draftData.title}}"</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="edit-cover-image-container fine-box">
                            <div v-if="!editMode || editMode && editingCurrent !== 'cover-image'"  class="render" title="Click To Edit This Section" @click="goToEditMode('cover-image')">
                                <div class="no-image-container">
                                    ...
                                    <p class="center"><strong>File :</strong> {{ draftData.coverimage?.name + draftData.coverimage?.extension || 'Not yet uploaded' }}</p>
                                    <p><strong>Cover Image Credits : </strong> {{draftData.coverimage?.credits || 'No credits for this image'}}</p>
                                </div>
                            </div>
                            <div v-if="editMode && editingCurrent == 'cover-image'"  class="edit">
                                <div class="border p-2 pb-3">
                                    ...
                                    <input type="file" id="cover_input" class="updateCoverInput" accept="image/*">
                                </div>
                            ...
                            <vs-dialog v-model="update_cover_warning">
                                <div>You need to upload an image before proceeding to save</div>
                            </vs-dialog>
                        </div>
                        <div class="edit-details-container fine-box">
                            <!--cartegory and published details-->
                            <div class="render" title="Click To Edit This Section" @click="goToEditMode('details')">
                    ...
                    <div class="pt-5 pb-2 center bold-font">
                        Add and Edit The Blog Contents Below <i class="fas fa-chevron-circle-down"></i>
                    </div>
                    <div class="article-main-content-section" v-for="section in sectionsArray" :key="section.sid">
                        ...
                        <div class="edit-image-box-container fine-box" v-if="section.type == 'image'">
                            <div  v-if="!editMode || editMode && editingCurrent !== section.sid"  title="Click To Edit This Section" class="render" @click="goToEditMode(section.sid,section.content)">
                                <div class="edit-image-info-container">
                                    ...
                                    <p class="">File : {{section.content.name}}{{section.content.extension}}</p>
                        ...
                        <div class="edit-paragraph-box-container fine-box" v-if="section.type == 'paragraph'">
                            ...
                            <div v-if="editMode && editingCurrent == section.sid"  class="edit">
                                <div>
                                    ...
                                    <vs-button block class="no-margin" warn flat @click="updateText(draftData.contents[section.sid].content, section.sid)">Save Updated Paragraph</vs-button>
                                </div>
                            </div>
                        </div>
                        <div class="edit-subheading-box-container fine-box subheading-edit" v-if="section.type == 'subheading'">
                            ...
                            <div v-if="editMode  && editingCurrent == section.sid"  class="edit">
                                <div>
                                    <textarea v-model="draftData.contents[section.sid].content" placeholder="Update Post Title Here" class="block-edit-input"></textarea>
                                    <vs-button block class="no-margin" warn flat @click="updateText(draftData.contents[section.sid].content, section.sid)">Save Updated Subheading</vs-button>
                                </div>                    
                            </div>
                        </div>
                        ...
                        <vs-dialog prevent-close v-model="previewCompletedModalActive">
                                <div class="center">
                                    <p class="py-2">Rendered the preview successfully</p>
                                    <p><a target="_blank" :href="previewLink" class="text-body-bold">View Preview</a></p>
...
import {getDraft,editDraft,managePostDelete,renderPreview,managePostUnpublish,managePostPublish} from "../utils/requests"
export default {
    data : function () {
        return {
            name : "edit-post",
    ...
    methods : {
       ...
       copyPreviewLink(){
        ...
        addSectionInPlace(type,where){ 
            ...
            let itemIndex = this.draftData.order.indexOf(theSID)
            let data;
            if(type == "image"){
                data = {
                    type : "image",
            ...
            this.saveNewSectionOrder(data,this.draftData.order)
            this.otherEditActive = false;
        },
        
        saveNewSectionOrder(data,order){
            editDraft(this.blogid,this.aid,{type: "updateOrder",data,order})
            .then((response)=>{
        ...
        saveNewSection(data){
            editDraft(this.blogid,this.aid,{type: "addSection", data})
            .then((response)=>{
                console.log("response", response.data);
            }).catch((error)=>{
                console.error("Error : ",error);
        ...
        addImage () {
            ...
            this.saveNewSection(imageSectionData)
        ...
        addParagraph : function () {
            ...
            this.saveNewSection(paraSectionData);
        },
        addList : function () {
            ...
            this.saveNewSection(listSectionData);
        },
        addSubheading : function () {
            ...
            this.saveNewSection(subHeadingSectionData);
        },
        updateText(content,sid){
            let data = {value: content,sid,type: 'updateText'}
            console.log("SID", sid)
            editDraft(this.blogid,this.aid,data)
            .then((res)=>{
        ...
        saveList (content,sid) {
            editDraft(this.blogid,this.aid,{sid,value:content,type: "updateList"})
            .then((response)=>{
                console.log("response", response.data);
                this.closeEditMode()
                this.draftData.contents[sid].content = content;
                this.updateView();
            }).catch((error)=>{
        ...
        saveTitle (newTitle) {
            editDraft(this.blogid,this.aid,{type: "updateTitle",value: newTitle})
            .then(()=>{
                this.closeEditMode()
            }).catch((error)=>{
                console.error("Error : ",error);
        ...
        updateCoverInfo(){
            ...
            let coverUpdateInput = document.getElementById("cover_input")
            if(coverUpdateInput.files.length == 0){
                this.update_cover_warning = true
                return
            }
            let file = coverUpdateInput.files[0]
            let formData = new FormData()
            formData.append("file",file)
            formData.append("credits",credits)
            formData.append("type","updateCoverImage")
            let headers = {
                "Content-Type" : "multipart/form"
            }
            editDraft(this.blogid,this.aid,formData,headers)
            .then((res)=>{
                console.log(res.data)
                this.draftData.coverimage = res.data.update
                this.closeEditMode()
            })
            .catch((err)=>{
        ...
        updateCategory () {
            editDraft(this.blogid,this.aid,{category: this.category,type: "updateCategory"})
            .then(res=>{
                console.log("res", res)
                this.draftData.category = this.category
                this.closeEditMode()
            })
            .catch(err=>{
        ...
        updatePostImageSection (sid) {
            ...
            console.log(sid,currentImgCredits,file);
            let formData = new FormData();
            ...
            .then(res=>{
                ...
                this.draftData.contents[sid].content = res.data.update
                this.updateView();
                this.closeEditMode()
            })
            .catch((err)=>{
        ...
        deleteSection(sid){
            ...
            let itemIndex = this.draftData.order.indexOf(sid)
            this.draftData.order.splice(itemIndex,1);
            this.draftData.contents[sid] = null;
            editDraft(this.blogid,this.aid,{type: "deleteSection",order: this.draftData.order})
            .then(res=>{
        ...
        previewDraft(){
            console.log("previewing draft file",this.aid)
            renderPreview(this.blogid,this.aid,{aid: this.aid})
            .then(res=>{
                console.log(res.data)
        ...
        publishDraft(){
            console.log("publishing the draft file",this.id)
            managePostPublish(this.blogid,this.aid,{aid: this.aid})
            .then(response=>{
        ...
        unpublishDraft(){
            ...
            managePostUnpublish(this.blogid,this.aid,{aid: this.aid})
            .then(res=>{
        ...
        deleteDraft(){
            console.log("delete functionality not yet enabled")
            managePostDelete(this.blogid,this.aid,{aid: this.aid})
            .then(res=>{
                console.log(res.data)
                this.$vs.notification({
    ...
    created : function () {
        getDraft(this.blogid,this.category,this.aid).then(res=>{
            console.log("res", res.data)
            this.draftData = res.data.draft_data
            this.published()
            this.created_init()
        }).catch(error=>{
